...
class NonResidentTypeControllerSpec extends SpecBase {
  ...
  lazy val nonresidentTypeRoute = routes.NonResidentTypeController.onPageLoad(fakeDraftId).url
  ...
  "Non-residentType Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(NonResidentType.values.head), fakeDraftId)(request, messages).toString
